...
const SettingsDialog: React.FC = observer(() => {
  ...
  return (
    ...
    >
      <PadlessDialogTitle>
        <AppBar position="static" color="primary">
          <Tabs
            ...
            variant="fullWidth"
            onChange={(_, newValue: DisplayMode) =>
              settingsDialogStore.setDisplayMode(newValue)
            }
          >
            <Tab
      ...
      <DialogContent>
        <StyledBackdrop
          open={settingsDialogStore.status === StoreStatus.working}
        >
          <CircularProgress color="inherit" />
        </StyledBackdrop>

        <Box
          hidden={settingsDialogStore.displayMode !== DisplayMode.primary}
          position="relative"
        ></Box>
        <Box
          hidden={settingsDialogStore.displayMode !== DisplayMode.workspaces}
          position="relative"
        >
          <List>
            {workspacesStore.workspaces.slice().map((item) => (
              <ListItem key={item.id}>
                <ListItemText
                  primary={item.name}
                  secondary={
                    workspacesStore.currentWorkspace?.id == item.id
                      ? t("settings.current_workspace")
                      : undefined
                  }
                />
