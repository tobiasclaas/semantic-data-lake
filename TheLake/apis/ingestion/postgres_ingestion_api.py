...
class PostgresIngestion(Resource):
    def __ingest(
        self, apiUser, host, port, database, table, dbUser, dbPassword, targetStorage, 
        ...
        with app.app_context():
            ...
            try:
                task = f"Ingestion in data lake of {database}.{table} from: {host}:{port}"
                job = create_job(jobId, task, apiUser)
                sparkHelper = SparkHelper(job)
                dataframe = sparkHelper.readPostrges(
                        host, port, dbUser, dbPassword, database, table
                )
                ...
                target_url, mime_type, uid = write_to_datalake(
                    dataframe, sparkHelper, SourceTypes.POSTGRES, targetStorage
                )
                save_db_metadata(
                    uid, dataframe, f"{host}:{port}/{database}", dbUser, dbPassword, database, table, 
                    mime_type, comment, apiUser, targetStorage, target_url, humanReadableName,
                    MetaData
                )
                job_succeeded(job)
    ...
    @parseParams(
        ...
        Argument("targetStorageSystem", default='PostgreSQL', type=str, required=False),
    ...
    def post(
        self, host, port, database, table, targetStorageSystem, user, password, comment,
        humanReadableName
    ):
        ...
        scheduler.add_job(
            lambda: self.__ingest(
                apiUser, host, port, database, table, user, password, targetStorageSystem,
                comment, humanReadableName, jobId
            ),
        )
        scheduler.start()
